...
export function ArticleFormDialog({ isOpen, onOpenChange, article, stores }: { isOpen: boolean, onOpenChange: (open: boolean) => void, article?: InventoryItem | null, stores: Store[] }) {
  ...
  useEffect(() => {
    if (privateData) {
        form.setValue('cost', privateData.cost);
    }
  }, [privateData, form])
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
        ...
        const itemData: Omit<InventoryItem, 'id' | 'createdAt' | 'updatedAt'> = {
            ...
            stockTotal: isNewArticle ? values.initialStock : (article!.stockReserved + article!.stockAvailable),

            minStock: values.minStock,
            idealStock: values.idealStock,
            dimensions: { length: values.length || 0, width: values.width || 0, height: values.height || 0 },
            photos: photoUrls,
            expirationDate: values.expirationDate ? Timestamp.fromDate(new Date(values.expirationDate)) : null,
            category: values.categoryId || '',
            description: values.description || '',
            barcode: values.barcode || '',
            packageLocation: values.packageLocation as PackageLocation || null,
            warehouseSubLocation: values.warehouseSubLocation as WarehouseSubLocation || null,
