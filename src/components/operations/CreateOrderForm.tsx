...
export default function CreateOrderForm({ open, onOpenChange, onOrderCreated }: CreateOrderFormProps) {
  ...
  const stores = useMemo(() => (storesData || []).filter(s => s.status === 'active'), [storesData]);
  ...
  const inventoryItems = useMemo(() => (inventoryData || []).filter(i => i.status === 'active'), [inventoryData]);
  ...
  const handleSubmit = () => {
    ...
    const serviceFee = serviceType !== 'fulfillment' ? rates.serviceFee : 0;
                ...
                <div className="bg-muted/50 rounded-lg p-3 space-y-1 text-sm">
                  ...
                  <div className="flex justify-between font-medium">
                    ...
                    <span>RD$ {(
                      ...
                      (serviceType !== 'fulfillment' ? selectedStore.rates.serviceFee : 0)
                    ).toLocaleString()}</span>
                  </div>
                </div>
              </div>
